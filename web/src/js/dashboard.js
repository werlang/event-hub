...
import { countEvents, createSummaryCards, createSummaryCardElement } from './dashboard/summary-cards.js';
...
class DashboardPage extends BaseComponent {
    ...
    async refreshEvents({ showErrors = false } = {}) {
        ...
        const response = await eventApi.listMine(this.#session.token);
    ...
    async #approveEvent(event, eventCard) {
        ...
        try {
            const response = await eventApi.moderate(this.#session.token, event.id, { status: 'published' });
    ...
    /**
     * Loads the event source associated with the active moderation scope.
     */
    #loadModerationEvents() {
        if (isPublishedModerationScope(this.#moderationScope)) {
            return eventApi.listPublic(null, { token: this.#session.token });
        }

        if (isRejectedModerationScope(this.#moderationScope)) {
            return eventApi.listModeration(this.#session.token, { status: 'rejected' });
        }

        return eventApi.listModeration(this.#session.token, { status: 'pending' });
    }
    ...
    async refreshModerationEvents({ showErrors = false } = {}) {
        ...
        const response = await this.#loadModerationEvents();
